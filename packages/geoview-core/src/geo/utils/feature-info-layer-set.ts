...
export class FeatureInfoLayerSet {
  ...
  private constructor(mapId: string) {
    ...
    const registrationConditionFunction = (layerPath: string): boolean => {
      const layerConfiguration = api.maps[this.mapId].layer.registeredLayers[layerPath];
      if (layerConfiguration?.source) {
        return 'featureInfo' in layerConfiguration.source! && !!layerConfiguration.source.featureInfo?.queryable;
      }
      return false;
    };
